...
function authorize (options) {
  ...
  var getOauthToken = function (id, secret, callback) {
    ...
    request(url, function (err, res, body) {
      ...
      if (res.statusCode === 200) {
        ...
        serverErr = new error.ServerError(body, res.statusCode);
        utility.logsAsync(serverErr);
  ...
  var validateToken = function (myToken, tokenToValidate, permissions, cache, callback) {
    ...
    var uri = options.oauth2.validate
      ...
      .replace('{a}', permissions.join(',')),
        url = 'https://' + options.oauth2.host + uri,
    ...
    request(
      ...
      function (err, res, body) {
        ...
        if (res.statusCode === 200) {
          ...
          validateTokenError = new error.ServerError(res.body, res.statusCode);
          utility.logsAsync(validateTokenError);
          // If local token is invalid, remove from cache
          if (res.statusCode === 401 || res.statusCode === 403) {
            cache.del('MY_TOKEN');
          }

          return callback(validateTokenError);
        }
      }
    ); // request(
  ...
  return function (req, res, next) {
    ...
    var onValidateToken = function (err, result) {
      ...
      if (err)
        return sendError(error.wrap(err, 500));
...
function enforceSsl (options) {
  ...
  return function (req, res, next) {
    ...
    var httpsProtoError = new error.BadRequest('Use HTTPS for all requests');
    ...
    if (req.headers['x-forwarded-proto'] !== 'https') {
      utility.logsAsync(httpsProtoError);
      return res.status(httpsProtoError.status).send(httpsProtoError);
    }
    return next();
  };
} // function enforceSsl (options) {
