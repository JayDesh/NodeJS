...
psku.Name as productdescription,
psku.usage_type__c,
...
psku.Deployment__c,
psku2.Deployment__c as deployment,
psku2.License_Model__c as license_model,
psku.Product_Line_code__c,
psku.Special_Sales_Program_Type__c,
sc1.Billing_Behavior_Code__c,
sc1.Billing_Behavior__c,
psku2.subscription_level__c as licensesku_subs_level
from salesforce.opportunitylineitem oli
LEFT JOIN salesforce.opportunity o ON o.sfid = oli.opportunityId
LEFT JOIN salesforce.PricebookEntry pbe ON pbe.sfid = oli.PricebookEntryId
LEFT JOIN salesforce.Product2 p2 ON p2.sfid = pbe.Product2Id
LEFT JOIN salesforce.contractlineitem cli ON cli.sfid = oli.Contract_Line_Item__c
LEFT JOIN salesforce.Asset__c ast ON ast.sfid = cli.Asset__c
LEFT JOIN salesforce.Contact ct ON ct.sfid = ast.Contact__c
LEFT JOIN salesforce.Account act ON act.sfid = ct.AccountId
LEFT OUTER JOIN salesforce.Entitlement ent ON ent.sfid = cli.Entitlement__c
LEFT JOIN salesforce.Product2 psku ON psku.sfid = cli.product_sku__c
LEFT JOIN salesforce.Product2 psku2 ON ast.part_number__C = psku2.part_number__C
LEFT JOIN salesforce.ServiceContract sc1 ON sc1.sfid = cli.ServiceContractId
where oli.opportunity__opportunity_number__c = $1